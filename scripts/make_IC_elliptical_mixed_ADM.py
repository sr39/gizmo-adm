...
def make_IC():
    ...
    to whatever IC you need
    '''
    fname='/tigress/sr39/gizmo_adm_testing/mixed_adm_v1/all_baryon_version/gas_cloud_elliptical_baryon_IC.hdf5'; # output filename
    ...
    for qi in qinit[:]:
        if np.dot(qi,qi) <= rd**2:
            ...
            q_adm.append(qi)
    q = np.asarray(q)
    q_adm = np.asarray(q_adm)

    # The final number of gas particles of each type
    Ngas = q.shape[0]
    ...
    y_adm = q_adm[:,1]
    z_adm = q_adm[:,2]
    r_adm = np.sqrt(x_adm**2 + y_adm**2 + z_adm**2)
    r_adm_matrix = np.zeros((Ngas,3))
    r_adm_matrix[:,0] = r_adm; r_adm_matrix[:,1] = r_adm; r_adm_matrix[:,2] = r_adm
    s_adm = np.sqrt(y_adm**2 + z_adm**2)
    ...
    lx_adm = s_adm/r_adm
    ly_adm = (1/r_adm)*(-np.sqrt(r_adm**2 - s_adm**2)*(y_adm/s_adm)*(x_adm/np.abs(x_adm)))
    lz_adm = (1/r_adm)*(-np.sqrt(r_adm**2 - s_adm**2)*(z_adm/s_adm)*(x_adm/np.abs(x_adm)))
    ...
    velocity_n_adm = np.cross(l_adm,q_adm/r_adm_matrix,axis=1)
    ...
    vvals_adm = np.sqrt(G* M(r_adm)/r_adm)
    vmatrix_adm = np.zeros((Ngas,3))
    vmatrix_adm[:,0] = vvals_adm; vmatrix_adm[:,1] = vvals_adm; vmatrix_adm[:,2] = vvals_adm;

    v_adm = velocity_factor*vmatrix_adm * (velocity_n_adm + np.random.rand(Ngas,3)*white_noise_factor)

    # Combining both velocities and both positions to generate our final values
    q = np.concatenate((q,q_adm))
    v = np.concatenate((v,v_adm))
    ...
    id_g=np.arange(1,2*Ngas + 1)

    # Set the ADM gas types here. I'll set all the gas particles to ADM particles. Need to have ADM flag enabled on GIZMO
    #id_adm = np.concatenate((np.zeros(Ngas,int),np.ones(Ngas,int)))
    id_adm = np.concatenate((np.zeros(Ngas,int),np.zeros(Ngas,int)))
    ...
    #
    npart = np.array([2*Ngas,0,0,0,0,0]) # we have gas and particles we will set for type 3 here, zero for all others
    ...
    b=np.zeros((2*Ngas,3)); b[:,0]=bx_g; b[:,1]=by_g; b[:,2]=bz_g;
