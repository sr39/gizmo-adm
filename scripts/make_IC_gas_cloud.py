...
def array_generator(xi, xf, n):
    array = np.arange(xi, xf, 1/n)
    array += 0.5*(xf - array[-1])
    return array

r0=array_generator(0,1,N_1D)
theta0 = array_generator(0,1,N_1D)*np.pi
phi0 = array_generator(0,1,N_1D)*2*np.pi

rv_g, tv_g, pv_g = np.meshgrid(r0,theta0,phi0, sparse=False)

xv_g = (rv_g*np.sin(tv_g)*np.cos(pv_g)).flatten()
yv_g = (rv_g*np.sin(tv_g)*np.sin(pv_g)).flatten()
zv_g = (rv_g*np.cos(tv_g)).flatten()

Ngas = xv_g.size

vx_g = np.random.randn(Ngas) * vgrainrms/np.sqrt(3.)
vy_g = np.random.randn(Ngas) * vgrainrms/np.sqrt(3.)
vz_g = np.random.randn(Ngas) * vgrainrms/np.sqrt(3.)


fig = plt.figure()
ax = Axes3D(fig)

ax.scatter(xv_g,yv_g,zv_g,'.')
plt.show()
"""
...
def make_IC():
    ...
    fname='gas_cloud.hdf5'; # output filename
    ...
    vx_g = np.random.randn(Ngas) * vgasrms/np.sqrt(3.)
    vy_g = np.random.randn(Ngas) * vgasrms/np.sqrt(3.)
    vz_g = np.random.randn(Ngas) * vgasrms/np.sqrt(3.)
    ...
    mv_g=rho_desired/((1.*Ngas)/(4*np.pi*(Lbox**3)/3)) + 0.*xv_g
    ...
    ax.scatter(vx_g,vy_g,vz_g,marker='.')
    plt.show()
    """
